...
class GridVisualizer:
    ...
    def create_animation(self, output_file=None):
        ...
        # Configurar el mapa de colores: -1 -> azul, 1 -> rojo
        cmap = plt.cm.RdBu
        vmin, vmax = -1, 1
        ...
        blue_patch = mpatches.Patch(color='blue', label='Estado -1')
        red_patch = mpatches.Patch(color='red', label='Estado 1')
        ax.legend(handles=[blue_patch, red_patch], loc='upper right')

        # Título inicial
        title = ax.set_title(f'Paso de Monte Carlo: {self.mcs_numbers[0]}')
    ...
    def create_snapshot(self, mcs_step, output_file=None):
        ...
        fig, ax = plt.subplots(figsize=(10, 10))
        im = ax.imshow(self.frames[mcs_step], cmap=plt.cm.RdBu, vmin=-1, vmax=1)
        ...
        ax.set_title(f'Paso de Monte Carlo: {self.mcs_numbers[mcs_step]}')
    ...
    def calculate_magnetization(self):
        ...
        magnetizations = [np.mean(frame) for frame in self.frames]
